...
CREATE OR REPLACE PROCEDURE REPORTE_RESERVA(FECHA_INICIO IN TIMESTAMP, FECHA_TERMINO IN TIMESTAMP, IDENTIFICADOR IN INT, NIVEL IN INT, R OUT SYS_REFCURSOR)
    ...
    END;
/
create or replace PROCEDURE REPORTE_STATS(P_FECHA_INICIO IN TIMESTAMP, P_FECHA_TERMINO IN TIMESTAMP, IDENTIFICADOR IN INT, NIVEL IN INT, R OUT SYS_REFCURSOR)
    IS
    V_T INT;
    BEGIN   
        IF NIVEL = 0 THEN
            OPEN R FOR
                SELECT NOMBRE_DPTO, SUM(VALOR_TOTAL), NVL(SUM(COSTO_MANTENCION),0), NVL(SUM(extract(day from FECHA_TERMINO - FECHA_INICIO)),0), NVL(SUM((SELECT VALOR FROM MULTA MUL WHERE MUL.ID_RESERVA = RES.ID_RESERVA)),0)
