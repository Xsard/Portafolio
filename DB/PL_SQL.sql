...
END P_CheckList;
/
CREATE OR REPLACE PROCEDURE REPORTE_RESERVA(FECHA_INICIO IN TIMESTAMP, FECHA_TERMINO IN TIMESTAMP, IDENTIFICADOR IN INT, NIVEL IN INT, R OUT SYS_REFCURSOR)
    IS
    BEGIN   
        IF NIVEL = 0 THEN
            OPEN R FOR
                SELECT NOMBRE_DPTO, COUNT(ID_DPTO), round(avg(extract(day from CHECK_OUT - CHECK_IN))), COUNT((SELECT ID_RESERVA FROM MULTA MUL WHERE MUL.ID_RESERVA = RES.ID_RESERVA))
    ...
    END;
/
CREATE OR REPLACE PROCEDURE REPORTE_STATS(FECHA_INICIO IN TIMESTAMP, FECHA_TERMINO IN TIMESTAMP, IDENTIFICADOR IN INT, NIVEL IN INT, R OUT SYS_REFCURSOR)
    IS
    BEGIN   
        IF NIVEL = 0 THEN
            OPEN R FOR
                SELECT NOMBRE_DPTO, SUM(VALOR_TOTAL), NVL(SUM(COSTO_MANTENCION),0), NVL(SUM(extract(day from FECHA_TERMINO - FECHA_INICIO)),0), SUM((SELECT VALOR FROM MULTA MUL WHERE MUL.ID_RESERVA = RES.ID_RESERVA))
                    FROM DEPARTAMENTO LEFT JOIN MANTENIMIENTO USING(ID_DPTO) LEFT JOIN RESERVA RES USING(ID_DPTO) WHERE CHECK_IN >= FECHA_INICIO AND CHECK_OUT <= FECHA_TERMINO  GROUP BY NOMBRE_DPTO;
        ELSIF NIVEL = 1 THEN
            OPEN R FOR
                SELECT NOMBRE_DPTO, SUM(VALOR_TOTAL), NVL(SUM(COSTO_MANTENCION),0), NVL(SUM(extract(day from FECHA_TERMINO - FECHA_INICIO)),0), SUM((SELECT VALOR FROM MULTA MUL WHERE MUL.ID_RESERVA = RES.ID_RESERVA))
                    FROM DEPARTAMENTO LEFT JOIN MANTENIMIENTO USING(ID_DPTO) LEFT JOIN RESERVA RES USING(ID_DPTO) JOIN COMUNA USING (ID_COMUNA) WHERE CHECK_IN >= FECHA_INICIO AND CHECK_OUT <= FECHA_TERMINO AND ID_REGION = IDENTIFICADOR GROUP BY NOMBRE_DPTO;            
        ELSIF NIVEL = 2 THEN
            OPEN R FOR
                SELECT NOMBRE_DPTO, SUM(VALOR_TOTAL), NVL(SUM(COSTO_MANTENCION),0), NVL(SUM(extract(day from FECHA_TERMINO - FECHA_INICIO)),0), SUM((SELECT VALOR FROM MULTA MUL WHERE MUL.ID_RESERVA = RES.ID_RESERVA))
                    FROM DEPARTAMENTO LEFT JOIN MANTENIMIENTO USING(ID_DPTO) LEFT JOIN RESERVA RES USING(ID_DPTO) WHERE CHECK_IN >= FECHA_INICIO AND CHECK_OUT <= FECHA_TERMINO AND ID_COMUNA = IDENTIFICADOR GROUP BY NOMBRE_DPTO;    
        ELSIF NIVEL = 3 THEN
            OPEN R FOR
                SELECT NOMBRE_DPTO, SUM(VALOR_TOTAL), NVL(SUM(COSTO_MANTENCION),0), NVL(SUM(extract(day from FECHA_TERMINO - FECHA_INICIO)),0), SUM((SELECT VALOR FROM MULTA MUL WHERE MUL.ID_RESERVA = RES.ID_RESERVA))
                    FROM DEPARTAMENTO LEFT JOIN MANTENIMIENTO USING(ID_DPTO) LEFT JOIN RESERVA RES USING(ID_DPTO) WHERE CHECK_IN >= FECHA_INICIO AND CHECK_OUT <= FECHA_TERMINO AND ID_DPTO = IDENTIFICADOR GROUP BY NOMBRE_DPTO; 
        END IF;
    END;
/
