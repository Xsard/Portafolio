...
CREATE OR REPLACE PROCEDURE REPORTE_RESERVA(FECHA_INICIO IN DATE, FECHA_TERMINO IN DATE, IDENTIFICADOR IN INT, NIVEL IN INT, R OUT SYS_REFCURSOR)
    ...
    BEGIN   
        IF NIVEL = 0 THEN
            OPEN R FOR
                SELECT NOMBRE_DPTO, COUNT(ID_DPTO), round(avg(extract(day from TO_DATE(CHECK_OUT - CHECK_IN,'dd-MM-yy')))), COUNT((SELECT ID_RESERVA FROM MULTA MUL WHERE MUL.ID_RESERVA = RES.ID_RESERVA))
...
create or replace PROCEDURE REPORTE_STATS(P_FECHA_INICIO IN DATE, P_FECHA_TERMINO IN DATE, IDENTIFICADOR IN INT, NIVEL IN INT, R OUT SYS_REFCURSOR)
    ...
    BEGIN   
        IF NIVEL = 0 THEN
            OPEN R FOR
                SELECT NOMBRE_DPTO, SUM(VALOR_TOTAL), NVL(SUM(COSTO_MANTENCION),0), NVL(SUM(extract(day from TO_DATE(CHECK_OUT - CHECK_IN,'dd-MM-yy'))),0), NVL(SUM((SELECT VALOR FROM MULTA MUL WHERE MUL.ID_RESERVA = RES.ID_RESERVA)),0)
